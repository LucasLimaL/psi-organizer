...
| **Bloqueio ADMIN** (manual) → `401 BLOQUEADO_ADMIN` no login (recusa dura). **Bloqueio INADIMPLENCIA** → login entra em **modo restrito**: token com claim `restrito=true`, e o `JwtAuthFilter` só libera `/me/assinatura` + `/me/renovar-sessao`; resto → `403 CONTA_BLOQUEADA`. Enforcement pelo claim, **sem query por request** (defasagem ≤24h aceita). Detalhe no fim do §11. | `AuthService.autenticar` · `JwtAuthFilter` |
...
| **Situação irregular** = sem contrato vigente OU fatura vencida. Avaliada no **login**: bloqueia automaticamente (motivo `INADIMPLENCIA`) e **emite token em modo restrito** (`restrito=true`) — o frontend leva o psicólogo direto pra `/assinatura`, única tela acessível até regularizar. | `AuthService.autenticar` · `LoginPage` |
...
### Acesso restrito por inadimplência (✅ implementado · 2026-06-13)

Mini-ADR, implementado. Substitui, para o motivo `INADIMPLENCIA`, o antigo comportamento de "bloqueio = `401` no login" (o motivo `ADMIN` não muda).

- **Decisão.** Conta bloqueada por **`INADIMPLENCIA`** **consegue logar**, porém em **modo restrito**: todas as funcionalidades bloqueadas, **exceto a tela `/assinatura`**. Bloqueio **`ADMIN`** continua como rejeição dura no login (`401 BLOQUEADO_ADMIN`) — é um "não" deliberado de um humano, não um funil de pagamento.
- **Mecanismo — claim no JWT, sem query por request (Opção A).** O login calcula a situação e, se irregular, emite o token com o claim booleano `restrito`. O `JwtAuthFilter` lê o claim e libera apenas um **allowlist** — `/me/assinatura` (e sub-recursos) e `POST /me/renovar-sessao`; qualquer outro endpoint → **`403 CONTA_BLOQUEADA`** com `detalhes.motivo`. As rotas públicas `/auth/**` (re-login/signup) **não** entram no enforcement — funcionam mesmo com um token restrito antigo anexado (o frontend manda o `Authorization` em toda chamada). Logout é client-side. O 403 é emitido **fora** do try/catch do filtro (fail-closed: falha ao escrever não libera a request). Nenhuma ida ao banco por request.
...
- **Liberação após pagamento.**
  - **Agora (sem integração de pagamento):** botão **"Já paguei — liberar acesso"** na tela de Assinatura que chama `POST /me/renovar-sessao` (recalcula a situação e re-emite o token, sem pedir senha); regularizada, o claim `restrito` some e o acesso volta — sem esperar as 24h. (A regularização hoje vem do admin dando baixa na fatura; o botão faz a sessão refletir isso na hora.)
