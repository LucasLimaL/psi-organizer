...
- **Mecanismo — claim no JWT, sem query por request (Opção A).** O login calcula a situação e, se irregular, emite o token com o claim booleano `restrito`. O `JwtAuthFilter` lê o claim e libera apenas um **allowlist** — `GET /me/assinatura` (ver a situação) e `POST /me/renovar-sessao` (regularizar); qualquer outro endpoint → **`403 CONTA_BLOQUEADA`** com `detalhes.motivo`. Logout é client-side (não chama API). Nenhuma ida ao banco por request.
